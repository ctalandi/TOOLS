PROGRAM cdfbuoyflx
  ...
  DO jt = 1, npt
    ...
    ! wnet(:,:) = getvar(cf_flxfil, cn_sowaflup, 1, npiglo, npjglo, ktime=jt )*86400.*zmask(:,:)          ! mm/days
     wnet(:,:) = getvar(cf_flxfil, cn_sowaflup, 1, npiglo, npjglo, ktime=jt )*zmask(:,:)    ! kg/m2/s
     ...
     zcoefw(:,:)= Grav* zbeta * zsss / 1000 * 1.e6   ! division 1000 to get back water fluxes in m/s, depends on the read field units
     ...
     wdmp(:,:)= getvar(cf_flxfil, cn_sowafldp, 1, npiglo, npjglo, ktime=jt)*zmask(:,:) ! mm/days
     ...
     runoff(:,:)= getvar(cf_flxfil, cn_sorunoff , 1, npiglo, npjglo, ktime=jt)*zmask(:,:)         ! mm/days

     ! Precip:
     !CL
     !precip(:,:)= evap(:,:)-runoff(:,:)+wdmp(:,:)-wnet(:,:)+wice(:,:)                     ! mm/day
     precip(:,:) = getvar(cf_flxfil, cn_sowaflup, 1, npiglo, npjglo, ktime=jt )*zmask(:,:)          ! mm/days


     ! Precip+runoff : (as a whole ) (interpolated on line)
     !CL
     precip_runoff(:,:)= precip(:,:)+runoff(:,:)                          ! mm/day
     ...
     wice(:,:)=evap(:,:)+wdmp(:,:)-precip_runoff(:,:)-wnet(:,:)
  ...
  !    precip_runoff(:,:)= evap(:,:)+wdmp(:,:)-wnet(:,:)+wice(:,:)                          ! mm/day
     ...
     IF ( lsho ) THEN
        ...
        ierr = putvar(ncout, id_varout(2),bh_net     , 1,npiglo, npjglo, ktime=jt )
        ierr = putvar(ncout, id_varout(3),bw_net     , 1,npiglo, npjglo, ktime=jt )
        ierr = putvar(ncout, id_varout(4), zsss      , 1,npiglo, npjglo, ktime=jt )
        ierr = putvar(ncout, id_varout(5), zsst      , 1,npiglo, npjglo, ktime=jt )
     ELSE
...
CONTAINS
  SUBROUTINE CreateOutput
    ...
    IF ( lsho ) THEN
       ...
       ! SSS
       stypvar(4)%cname= 'sss'                      ;   stypvar(5)%cname= 'sst'
       stypvar(4)%cunits='PSU'                      ;   stypvar(5)%cunits='Celsius'
       stypvar(4)%rmissing_value=1.e+20                 ;   stypvar(5)%rmissing_value=1.e+20
       stypvar(4)%valid_min= 0.                     ;   stypvar(5)%valid_min= -2.
       stypvar(4)%valid_max= 45                     ;   stypvar(5)%valid_max= 45
       stypvar(4)%clong_name='Sea Surface Salinity' ;   stypvar(5)%clong_name='Sea Surface Temperature'
       stypvar(4)%cshort_name='sss  '               ;   stypvar(5)%cshort_name='sst'
