...
CONTAINS
  ...
  INTEGER(KIND=4) FUNCTION create( cdfile, cdfilref ,kx,ky,kz ,cdep, cdepvar, &
    ...
    IF ( PRESENT(cdlonvar) ) THEN
       ...
       istatus = copyatt(cdlonvar, nid_lon1d,incid,icout)
    ENDIF
    IF ( PRESENT(cdlatvar) ) THEN
       istatus = NF90_DEF_VAR(icout,cdlatvar,NF90_FLOAT,(/nid_y/), nid_lat1d)
       istatus = copyatt(cdlatvar, nid_lat1d,incid,icout)
    ENDIF
    IF ( kz /= 0 ) THEN
    ...
    istatus=NF90_PUT_ATT(icout,NF90_GLOBAL,'History', "To avoid an artificial weak curl at the north pole, duplicate the raw before the last onto the last raw ")
    istatus=NF90_PUT_ATT(icout,NF90_GLOBAL,'About', "Drowned field created by SOSIE interpolation environement (http://sosie.sourceforge.net/) / Laurent Brodeau, 2015" )
  ...
  INTEGER(KIND=4) FUNCTION createvar(kout, sdtyvar, kvar, kpk, kidvo, cdglobal, ld_nc4, infileatt)
    !!---------------------------------------------------------------------
    !!                  ***  FUNCTION createvar  ***
    !!
    ...
    CHARACTER(LEN=*), OPTIONAL,       INTENT(in) :: infileatt ! Input file from which to get the variable attributs
    LOGICAL         , OPTIONAL,       INTENT(in) :: ld_nc4  ! user chunking and deflation

    INTEGER(KIND=4)               :: jv             ! dummy loop index
    INTEGER(KIND=4)               :: idims, istatus, incid
    ...
    DO jv = 1, kvar
       ...
       IF ( sdtyvar(jv)%cname /= 'none' ) THEN
         ...
         ENDIF
          ...
          istatus = NF90_DEF_VAR(kout, sdtyvar(jv)%cname, iprecision, iidims(1:idims) ,kidvo(jv) )
#endif
          IF ( PRESENT ( infileatt ) ) THEN 
              istatus = NF90_OPEN(infileatt,NF90_NOWRITE,incid)
              istatus = copyatt(sdtyvar(jv)%cname, kidvo(jv),incid,kout)
          ELSE
              ! add attributes
