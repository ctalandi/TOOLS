    ...
    for i = 1:nlon
        for j = 1:nlat
            ...
            if sum(valid_idx) > 2  
                ...
                [SA_stab(t, valid_idx, j, i), CT_stab(t, valid_idx, j, i)] = ...
                    gsw_stabilise_SA_CT(SA_prof(valid_idx), CT_prof(valid_idx), p(valid_idx));
            end
        end
    end
end
...
% DONE % Création du fichier pour SA stabilisée
% DONE ncid_SA = netcdf.create(fullfile(dataDir, 'woa09_SalAbs_monthly_1deg_SA_CMA_drowned_Ex_L75_TS_Stabilise_NEW.nc'),'CLOBBER');
% DONE dimid_time = netcdf.defDim(ncid_SA, 'time_counter', nt);
% DONE dimid_depth = netcdf.defDim(ncid_SA, 'z', length(depth_profile));
% DONE dimid_lat = netcdf.defDim(ncid_SA, 'lat0', nlat);
% DONE dimid_lon = netcdf.defDim(ncid_SA, 'lon0', nlon);
% DONE varid_SA = netcdf.defVar(ncid_SA, 'SA', 'double', [dimid_lon, dimid_lat, dimid_depth, dimid_time]);
% DONE netcdf.endDef(ncid_SA);
% DONE % Note : L'ordre des dimensions doit correspondre à celui d'écriture (ici, [lon, lat, depth, time])
% DONE % Pour réarranger les données, on utilise permute :
% DONE netcdf.putVar(ncid_SA, varid_SA, SA_stab);
% DONE %netcdf.putVar(ncid_SA, varid_SA, permute(SA_stab, [4, 3, 2, 1]));
% DONE netcdf.close(ncid_SA);
% DONE disp('Fichier WOA09_SA_stab.nc créé.');

% Création du fichier pour CT stabilisée
ncid_CT = netcdf.create(fullfile(dataDir, 'woa09_ConTem_monthly_1deg_CT_CMA_drowned_Ex_L75_TS_Stabilise_NEW.nc'),'CLOBBER');
